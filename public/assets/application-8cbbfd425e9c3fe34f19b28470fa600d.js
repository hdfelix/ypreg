...
$(document).ready(function(){
	...
	function chartToggleSeries(placeholder) {
		...
		$.each( datasets, function( key, val ) {
			...
			choiceContainer.append(
				"<div class='simple-checkbox'><input type='checkbox' name='" + key +
	...
	if( $('#demo-d3-heatmap').length > 0 ) {
		...
		var dataHeat = "assets/js/stat/data-heatmap.tsv"; // path/to/your/datafile.tsv
	...
	if( $('#demo-d3-barchart').length > 0 ) {
		var dataBar = "assets/js/stat/data-barchart.tsv"; // path/to/your/datafile.tsv
...
$(document).ready(function(){
	...
	if( $('body').hasClass('text-editor') ) {
		...
		var initContent = '<h4>Hello there</h4> ' +
			'<p>How are you? I have bellow task for you :</p> ' + 

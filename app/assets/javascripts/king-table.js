$(document).ready(function(){
	...
	if( $('#jqgrid').length > 0 ) {
		...
		$('#jqgrid').jqGrid({
			...
			viewrecords: true,
			sortorder: "asc",
			editurl: "php/jqgrid/server.php",
			caption: "Working Editable Table",
			multiselect: true,
			onSelectRow: function(id){
				$("#"+id+"_invdate").datepicker({dateFormat:"m/d/Y"})
				.on('changeDate', function(){
					$(this).datepicker('hide'); // force close the calendar
				});
			}
		...
		if( $('#jqgrid').length > 0 ) {
			$('#jqgrid').jqGrid( 'navGrid', '#jqgrid-pager', { add:true, edit:true, view:true, del:true, search:true, refresh:true},
				{}, {}, {}, 
	...
	{
		if( $('#jqgrid').length > 0 ) {
			$('#jqgrid').fluidGrid({base:'#jqgrid-wrapper', offset:-20});
		}
	}
